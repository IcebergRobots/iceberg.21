...
Chassis::Chassis(Motor* motors, const int count, const bool& enabled)
  : _motors(motors), _motorCount(count)
{
  _enabled = enabled;
}
...
void Chassis::calculate(int angle, int power, int rotation)
{
  ...
  _motors[0].setPower(axis02 - rotation); //erstelle Zwischenspeicher für alle Motorstärken
  _motors[1].setPower(axis13 - rotation);
  _motors[2].setPower(axis02 + rotation);
  _motors[3].setPower(axis13 + rotation);
}
...
{
  for (byte i = 0; i < _motorCount; i++)
    _motors[i].brake(activ);
}

void Chassis::setEn(const bool& enabled)
{ 
  for (int i = 0; i < _motorCount; i++)
    _motors[i].setEn(enabled);
  _enabled = enabled;
}
